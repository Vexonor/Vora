...
function ToggleGroup({
  ...
  return (
    <ToggleGroupPrimitive
      ...
      data-size={size}
      className={cn(
        'group/toggle-group flex gap-0.5 items-center rounded-lg data-[variant=outline]:shadow-xs data-[variant=outline]:border data-[variant=outline]:p-0.5',
        className,
      )}
      {...props}
    >
      <ToggleGroupProvider value={{ variant, size }}>
        {props.type === 'single' ? (
          <ToggleGroupHighlightPrimitive className="bg-primary/10 border-2 border-primary rounded-md">
            {children}
...
function ToggleGroupItem({
  ...
  return (
    <ToggleGroupHighlightItemPrimitive
      value={props.value}
      className={cn(type === 'multiple' && 'border-2 border-primary rounded-md')}
    >
      <ToggleGroupItemPrimitive
        ...
        className={cn(
          ...
          'min-w-0 border-2 border-foreground/10 flex-1 shrink-0 shadow-none rounded-md focus:z-10 focus-visible:z-10',
          className,
        )}
        {...props}
      >
...
export {
  ToggleGroup,
  ToggleGroupItem, type ToggleGroupItemProps, type ToggleGroupProps
};
